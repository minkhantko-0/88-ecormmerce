...
import TypeEditLoading from '@/components/ui/custom/type/edittype.loading';
...
export default function Page() {
  ...
  const updateTypeMutation = useMutation(api.types.updateType);
  const deleteTypeMutation = useMutation(api.types.deleteType);

  // Fetch type data based on the URL id.
  const typeData = useQuery(api.types.getType, {
    id: typeId as Id<'types'>
  });
  ...
  // If there's no type data, render the loading skeleton.
  if (!typeData) {
    return <TypeEditLoading />;
  }

  // Save the original image URL for later comparison.
  const originalImageUrl = typeData.imageUrl || '';
  ...
  const handleDeleteType = async () => {
    try {
      ...
      toast.success('Type deleted successfully!', { richColors: true });
  ...
  // On form submit: optionally upload a new image, update the type, and delete the old image if needed.
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      toast.success('Type updated successfully!', { richColors: true });
  ...
  return (
    <div className='mx-auto max-w-md p-4'>
      <h1 className='mb-4 text-center text-2xl font-bold'>Edit Type</h1>
      <FormProvider {...form}>
